...
$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_TEMP_PLC)/VCShared/dso.Internal.vco: $(AS_PROJECT_PATH)/Logical/VCShared/DataSources/Internal.dso 
	 $(VCC) -f "$<" -o "$@" $(DSOFLAGS) $(VCCFLAGS_Visu) -p Visu -vcr 4211 -sfas
...
$(VCRS_OBJECT) : $(VCRS_SOURCE)
	$(VCC) -f "$<" -o "$@" -ct shared -cv "$(AS_PROJECT_PATH)/Logical/VCShared/ControlVersion.cvinfo" -sl  $(VCCFLAGS_Visu) -p Visu -vcr 4211 -sfas
...
$(SHARED_CCF): $(VCRS_OBJECT) $(VCR_OBJECTS_Visu) $(VCUG_OBJECTS_Visu) $(ALGRP_OBJECTS_Visu) $(ALCFG_OBJECTS_Visu) $(DSO_OBJECTS_Visu) $(TXTGRP_SHARED_OBJECTS_Visu) $(CVINFO_OBJECTS_Visu)
	-@CMD if exist /Q "$(DEL_SHARED_CCF)" /C DEL /F /Q "$(DEL_SHARED_CCF)" 2>nul
	 ...
	 $(LINK) "$@.lfl" -o "$@" -lib "$(LIB_SHARED)" -P "$(AS_PROJECT_PATH)" -m "shared resources" -profile "False" -warningLevel2 -name Visu -vcr 4211 -sfas
